...
@click.option('--url')
@click.option('--file')
@click.option('--plaintext')
def main(url, file, plaintext):
    if url:
    ...
    click.echo(f"Summarized text from -> {url or file or plaintext}")
    click.echo(process(text))


def summarizer(url=None, file= None, plaintext= None):
    if url:
        text = extract_from_url(url)
    elif file:
        with open(file, 'r') as _f:
